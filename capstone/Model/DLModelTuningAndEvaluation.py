...
def train_model(model,xtr,ytr,xte,yte,ep,optm="adam",bs=32,learn_rate=0.001,forTuning=True,callbacks=False,earlystop_params=None,reducelr_params=None,modelchkpnt_params=None):
  '''
  ...
  if(forTuning!=True):
    ...
    if(callbacks==True):
      chkpnt_path = "/content/drive/My Drive/capstone/Model/model checkpoints/"+modelchkpnt_params["chkpnt_filename"]+".h5"
...
def show_training(history, saveit=False, filename=None):
  ...
  if(saveit==True):
    if(filename==None):
      ...
      hist_df.to_csv('/content/drive/My Drive/capstone/Model/model histories/'+filename+'.csv',index=False)
